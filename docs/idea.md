...
*   **Step 2: The AI Creative Director (Concept Stress-Test)**
    *   **How it works:** Before presenting to the client, the freelancer picks a Concept Board from the dropdown and runs a critique on demand. The panel stays empty until a board is reviewed, so it is always clear which concept the arguments belong to.
        ...
        *   Reviewers can open the full concept snapshot via the built-in **View Details** button to double-check the version that will receive changes.
        *   A manual critique composer sits above the AI output so strategists can log bespoke notes even before running the generator; those notes join the board's Concept Critique history alongside AI-addressed arguments.
